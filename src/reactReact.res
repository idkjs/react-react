...
let componentFromSignal = (propsToVdom, props) => {
  ...
  React.useEffect0(() => {
    ...
    watcher.current = Some(S.map(newElement => setElement(_ => newElement), vdomS))
    Some(
      () => {
        /* Cleaning up resources */
        Belt.Option.map(propsPair.current, x => S.stop(~strong=true, x.signal)) |> ignore
...
module Utils = {
  ...
  module Event = {
    let join = ee => E.\"switch"(E.never, ee)
